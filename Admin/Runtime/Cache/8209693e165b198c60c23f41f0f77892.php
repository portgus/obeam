...
$(function(){
	$("#delete_all").click(function(){
	   ...
	   str= str.substr(0,str.length-1);   //分隔符占3个字符，截取字符串，去掉最后的"','"，这样正好匹配SQL语句
	    ...
	    $.ajax({
	        //url:"Delete.php",
	        url:"<?php echo U('Admin/FriendLink/delete_all');?>",
	        data:{'str':str},
	        type:"POST",
...
function trash_delete(id){
	$.ajax({
		url:"<?php echo U('Admin/FriendLink/delete');?>",
		data:{'id':id},
		type:'POST',
		success:function(data){
			if(data==1){
...
document.onkeydown=banBackSpace;
 ...
 <div class="centercontent tables">
  ...
  <div class="pageheader">
            <h1 class="pagetitle">友情链接管理</h1>
            <span class="pagedesc">FriendLink &nbsp;&nbsp;Manage</span>
                ...
                <table cellpadding="0" cellspacing="0" border="0" id="table2" class="stdtable stdtablecb">
                    ...
                    <thead>
                        <tr>
                            ...
                            <th class="head1">链接地址</th>
                            <th class="head0">管理</th>
                        </tr>
                    </thead>        
                    <tbody>
                    <?php if(is_array($linkArr)): foreach($linkArr as $k=>$v): ?><tr class="trdelete_<?php echo ($v['id']); ?>">
                        	<td align="center"><input type="checkbox" class="ck" value="<?php echo ($v['id']); ?>"/></td>
                            <td><?php echo ($v['id']); ?></td>
                            <td><?php echo ($v['website_name']); ?></td>
                            <td><?php echo ($v['link_url']); ?></td>
                            <td >
                       <a href="<?php echo U('Admin/FriendLink/add_link',array('id'=>$v['id']));?>" class="btn btn_mail"><span>编辑</span></a>
